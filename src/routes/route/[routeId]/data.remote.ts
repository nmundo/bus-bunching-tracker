...
export const getRouteStats = query(
	'unchecked',
	async ({ routeId, serviceId, bucket, directionId }: RouteInput): Promise<StatsResult> => {
...
export const getRouteSegments = query(
	'unchecked',
	async ({
		routeId,
		serviceId,
...
export const getRouteDirections = query(
	'unchecked',
	async ({ routeId }: { routeId: string }): Promise<Record<string, string>> => {
		try {
