...
const loadRouteMap = async () => {
  const result = await query<{ rt: string; gtfs_route_id: string }>('select rt, gtfs_route_id from route_map');
...
const loadPatternDirections = async () => {
  const result = await query<{ pid: string; dir: string | null }>('select pid, dir from bt_patterns');
...
export const runArrivals = async () => {
  ...
  for (const row of result.rows) {
    ...
    if (nextStop && state.lastPdist < nextStop.distance_feet && row.pdist_feet >= nextStop.distance_feet) {
      ...
      if (routeId && nextStop.gtfs_stop_id) {
        ...
        await query(
          ...
          [routeId, directionId, nextStop.gtfs_stop_id, row.vid, row.rt, row.pid, row.tmstmp, row.pdist_feet]
        );
      }
